...
for each_time = 1:num_times
    ...
    clearvars -except img_wells num_wells img_orgl img_bulbs img_area_log regs num_times each_time fbrgt_all fcell_all wells_disp_all sheet_names img_blob_area_all max_wells auto_illum conds num_conds curr_cond folder_name path_name git_path_name bbl_box
...
cd(git_path_name); 
if Ab_switch
    run('Step2a_Antibody_Analysis.m'); % cd into the condition folder
else
    fprintf('No antibody staining today! \n');
end
