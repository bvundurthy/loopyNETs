...
% clear variables
% clc
function [num_live_wells, cell_not_found, dead_wells_begin] = track_wells_initial_only(live_times, dead_times, live_sheets, dead_sheets, replicate_path_name)
    ...
    num_live_wells = length(data_live{1}(:,1)); % Total number of live cells in wells
    tracking_mat_wells = zeros(2*num_live_wells, 13); % Initializing the tracking matrix
    ...
    writematrix([num_live_wells, cell_not_found, dead_wells_begin], write_name, 'Sheet', 'wells_num','WriteMode','overwritesheet'); % includes the numbers
