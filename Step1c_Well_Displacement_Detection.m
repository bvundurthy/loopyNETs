...
fprintf('Starting Step 1c to detect wells displacement across all time points. Folder name: "%s" \n', path_name);
...
for each_time = 1:num_times
    ...
    if (curr_brgt_num==fbrgt_base_num) %when we are not dealing with base fbrgt
        fprintf('Image same as base image. \n'); 
        continue;
    end
    rep_fbrgt = find(fbrgt_num(1:(each_time-1))==curr_brgt_num);
    if ~isempty(rep_fbrgt)
        fprintf('Image already computed. Copying %d time well displacement into %d (current) time \n', rep_fbrgt, each_time);
        wells_disp_all(each_time,:) = wells_disp_all(rep_fbrgt,:);
        continue; 
    end
