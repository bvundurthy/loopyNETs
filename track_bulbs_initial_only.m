...
function [num_live_bulbs, cell_not_found, dead_bulbs_begin] = track_bulbs_initial_only(live_times, dead_times, live_sheets, dead_sheets, replicate_path_name)
    ...
    data_live{1} = readmatrix('Cells_Bulbs','sheet',live_sheets{1}); 
    for i = 1:dead_times
        data_dead{i} = readmatrix('Cells_Bulbs','sheet',dead_sheets{i});    
    end

    % If cells are too close in live image, ignore both of them
    %% Identifying the stage of cells throughout
    num_live_bulbs = length(data_live{1}(:,1)); % Total number of live cells in bulbs
    tracking_mat_bulbs = zeros(2*num_live_bulbs, 13); % Initializing the tracking matrix
    count = 0; dead_bulbs_begin = 0; cell_not_found = 0;  % Initializing count for tracking, and variables to identify dead cells in first time point and cells without a dead counterpart

    % Iterating over bulbs
            ...
            for each_time = 1:dead_times 
                ...
                if (isempty(idx) || any(cell_disp>25)) % Ensuring that it is indeed the same cell by restricting the distance between centroids to be no greater than 25 pixels (in both x and y)
                    ...
                    if (each_time~=1) % Ensuring that the cell died after time point 1
                        ...
                        tracking_mat_bulbs(count,7:12) = [each_time cells_time_well(idx,:)]; % Copying the dead cell with its well number, time of death, area and centroid into the tracking matrix
                        tracking_mat_bulbs(count,1:6) = [1 cells_curr_well(each_cell,:)];% Copying the live cell with its well number, time = 1, area and centroid into the tracking matrix
                    else % Cells at time point 1 are ignored
                        dead_bulbs_begin = dead_bulbs_begin + 1; % this variable counts all the cells that are dead at time point 1 
                    end
